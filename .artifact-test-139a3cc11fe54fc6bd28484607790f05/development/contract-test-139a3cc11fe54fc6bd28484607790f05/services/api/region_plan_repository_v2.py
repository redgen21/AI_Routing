...
POLICY_ALLOWLIST = frozenset({"assigned_region_boundary_spillover/v1", "explicit_workbook_membership/v1", "active_roster_type_hard_region_soft/v1", "active_roster_area_type_fallback_region_soft/v1"})
@dataclass(frozen=True)
    ...
    def review(self, r: Mapping[str, Any], **_):
        ...
        try:
            with conn.cursor() as c:
                self._begin(c,r)
                c.execute("select revision,activation_revision,plan_status,policy_version,source_sha256,manifest_sha256,bundle_sha256,membership_accepted_rows,membership_rejected_rows,unique_postal_count,technician_count,source_strategic_city_name from common_region_plan p join common_city_context cc using(subsidiary_name,strategic_city_name) where p.subsidiary_name=%s and p.strategic_city_name=%s and p.plan_id=%s for update",(identity.subsidiary_name,identity.strategic_city_name,identity.plan_id)); current=c.fetchone()
    ...
    def preview(self,r: Mapping[str,Any],**_):
        ...
        try:
            with conn.cursor() as c:
                ...
                c.execute("select plan_status,policy_version,source_sha256,manifest_sha256,bundle_sha256,membership_accepted_rows,membership_rejected_rows,unique_postal_count,technician_count,source_strategic_city_name from common_region_plan where subsidiary_name=%s and strategic_city_name=%s and plan_id=%s for share",(identity.subsidiary_name,identity.strategic_city_name,identity.plan_id)); stored=c.fetchone()
