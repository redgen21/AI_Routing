...
"${PYTHON_BIN}" sr_common_vrp_api_server.py --config "${CONFIG}" \
  --host 0.0.0.0 --port 8066 --expected-environment development --check-config
...
if wait_for_http_status "http://127.0.0.1:${PORT}/_stcore/health" 200 30 "${PID}"; then
  echo "Development Common VRP client server is ready on port ${PORT} (pid ${PID})."
else
  kill "${PID}" >/dev/null 2>&1 || true
  echo "Development client server failed readiness; see ${LOG_DIR}/client_server.err.log" >&2
  exit 1
fi
