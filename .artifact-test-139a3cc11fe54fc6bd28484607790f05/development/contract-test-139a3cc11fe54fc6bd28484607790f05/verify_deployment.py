...
def verify_deployment(config_path: Path, expected_environment: str) -> dict[str, object]:
    ...
    for config_key in ZCTA_CONFIG_PATHS:
        ...
        if not zcta_geometry.is_file():
            ...
            and zcta_geometry.resolve() != default_zcta_geometry.resolve()
        ):
            missing.append(
                f"zcta_geometry:{section_name} does not match the active production data catalog"
            )
    for config_key, default_role in AREA_MAP_CONFIG_PATHS.items():
        ...
        if not data_path.is_file():
            ...
            expected_environment == "production"
            and data_path.resolve() != catalog.resolve(default_role).resolve()
        ):
            missing.append(
                f"configured_data:{config_key} does not match the active production data catalog"
            )
