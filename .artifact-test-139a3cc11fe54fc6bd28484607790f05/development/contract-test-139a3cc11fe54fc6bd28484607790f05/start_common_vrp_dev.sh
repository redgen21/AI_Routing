...
HOST="${1:-0.0.0.0}"
PORT="${2:-8066}"
LOG_DIR="${ROOT}/log/common_vrp/dev"
...
"${PYTHON_BIN}" sr_common_vrp_api_server.py --config "${CONFIG}" \
  --host "${HOST}" --port "${PORT}" --expected-environment development --check-config
...
nohup "${PYTHON_BIN}" sr_common_vrp_api_server.py \
  --config "${CONFIG}" --host "${HOST}" --port "${PORT}" --expected-environment development \
...
if wait_for_http_status "http://127.0.0.1:${PORT}/api/v1/common/contexts" 200 30 "${PID}"; then
  echo "Development Common VRP API is ready on ${HOST}:${PORT} (pid ${PID})."
else
  kill "${PID}" >/dev/null 2>&1 || true
  echo "Development Common VRP API failed readiness; see ${LOG_DIR}/api.err.log" >&2
  exit 1
fi
