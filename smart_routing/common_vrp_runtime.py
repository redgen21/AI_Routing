...
def _build_payload_from_dataframes(
    ...
    # Sort technicians geographically (west→east by longitude, then south→north by latitude)
...
def submit_routing_from_inputs(
    ...
    routing_job_id = create_job_id(payload.get("request_id"))
    save_new_job(routing_job_id, payload)
