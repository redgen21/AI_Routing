...
PROFILE_FILE = na_data_path("profile_runtime")
...
CACHE_DIR = Path("data/cache/area_map_usa")
ROUTE_EXPLORER_CACHE_DIR = Path("data/cache/route_explorer_usa")
...
def _configured_service_file(config_file: Path = DEFAULT_CONFIG_FILE) -> Path | None:
    area_map_cfg = _load_json_config(config_file).get("area_map_usa", {})
...
def load_city_map_data(
    ...
    profile_path = _configured_area_map_path("profile_file", profile_path)
    zcta_zip_path = _configured_area_map_path("zcta_zip_file", zcta_zip_path)
...
def _build_integrated_zip_layer(postal_region_df: pd.DataFrame, service_df: pd.DataFrame) -> gpd.GeoDataFrame:
    zcta_zip_path = _configured_area_map_path("zcta_zip_file", ZCTA_ZIP_FILE)
