...
OWN_REGION_WITH_APPROVED_BOUNDARY_OVERFLOW_POLICIES = frozenset({
...
def _hard_eligible_employee_codes(job_row: pd.Series) -> set[str] | None:
    """Return an explicit hard candidate set, preserving an intentional empty set."""

    raw_codes = job_row.get("eligible_employee_codes")
...
def _uses_own_region_boundary_policy(region_policy: object) -> bool:
    return str(region_policy or "").strip() in OWN_REGION_WITH_APPROVED_BOUNDARY_OVERFLOW_POLICIES


def _approved_boundary_overflow_employee_codes(job_row: pd.Series) -> set[str]:
    """Return only approved overflow codes that are also hard-eligible."""

    raw_codes = job_row.get("boundary_overflow_employee_codes")
    if not isinstance(raw_codes, (list, tuple, set)):
        return set()
    approved_codes = {str(code).strip() for code in raw_codes if str(code).strip()}
    hard_codes = _hard_eligible_employee_codes(job_row)
    if hard_codes is None:
        return set()
    return approved_codes & hard_codes


def _fixed_technician_outside_active_plan(job_row: pd.Series, employee_code: object) -> bool:
    if not _uses_own_region_boundary_policy(job_row.get("region_policy")):
        return False
    code = str(employee_code or "").strip()
    hard_codes = _hard_eligible_employee_codes(job_row)
    return bool(code and hard_codes is not None and code not in hard_codes)


def _approved_boundary_overflow_penalty_cost(
    region_policy: object,
    employee_code: object,
    approved_employee_codes: set[str],
) -> int:
    """Apply overflow cost only to an explicitly approved boundary candidate."""

    if not _uses_own_region_boundary_policy(region_policy):
        return 0
    code = str(employee_code or "").strip()
    if not code or code not in approved_employee_codes:
        return 0
    return VRP_APPROVED_BOUNDARY_OVERFLOW_PENALTY_COST


def _preferred_region_mismatch_penalty_cost(
    preferred_region: object,
    job_region: object,
) -> int:
    preferred = str(preferred_region or "").strip()
    job = str(job_region or "").strip()
    return (
        VRP_PREFERRED_REGION_MISMATCH_PENALTY_COST
        if preferred and job and preferred != job
        else 0
    )
...
def _solve_vrp_day(
    ...
    enforce_area_type_center_match_by_job = [
        _coerce_bool_value(row.get("enforce_area_type_center_match", False))
        for _, row in job_df.iterrows()
    ]
    area_type_dms_fallback_allowed_by_job = [
        _coerce_bool_value(row.get("area_type_dms_fallback_allowed", False))
        for _, row in job_df.iterrows()
    ]
    region_names_by_job = (
    ...
    region_policy_by_job = (
        job_df.get("region_policy", pd.Series("", index=job_df.index))
        .fillna("")
        .astype(str)
        .str.strip()
        .tolist()
    )
    approved_boundary_overflow_codes_by_job = [
        _approved_boundary_overflow_employee_codes(row)
        for _, row in job_df.iterrows()
    ]
    ...
    if respect_fixed_jobs and "fixed" in job_df.columns:
        for fixed_job_idx, (_, fixed_row) in enumerate(job_df.iterrows()):
            ...
            is_fixed_outside_active_plan = _fixed_technician_outside_active_plan(
                fixed_row, fixed_employee_code
            )
            if (
                _coerce_bool_value(fixed_row.get("fixed", False))
                and fixed_vehicle_idx is not None
                and not is_fixed_outside_active_plan
            ):
                ...
                _coerce_bool_value(fixed_row.get("fixed", False))
                and fixed_vehicle_idx is not None
                and is_fixed_outside_active_plan
            ):
                # The current technician is available but outside the active
                # Atlanta plan.  Preserve the job's hard candidate set while
                # giving it the same protected, reschedule-like priority.
                reschedule_like_priority_job_nodes.add(int(fixed_job_idx))
    ...
    def _preferred_region_vehicle_penalty(vehicle_idx: int, to_node: int) -> int:
        ...
        return _preferred_region_mismatch_penalty_cost(preferred_region, job_region)

    def _approved_boundary_overflow_vehicle_penalty(vehicle_idx: int, to_node: int) -> int:
        if to_node >= job_count or not (0 <= vehicle_idx < len(vehicle_codes)):
            return 0
        region_policy = region_policy_by_job[to_node] if 0 <= to_node < len(region_policy_by_job) else ""
        approved_codes = (
            approved_boundary_overflow_codes_by_job[to_node]
            if 0 <= to_node < len(approved_boundary_overflow_codes_by_job)
            else set()
        )
        return _approved_boundary_overflow_penalty_cost(
            region_policy,
            vehicle_codes[vehicle_idx],
            approved_codes,
        )
    ...
    def _make_vehicle_cost_callback(vehicle_idx: int):
        def vehicle_cost_callback(from_index: int, to_index: int) -> int:
            ...
            return (
                int(round(_travel_minutes(from_node, to_node) * 100))
                + _long_leg_penalty_cost(from_node, to_node)
                + (_vehicle_priority_job_bias(vehicle_idx, to_node) if priority_load_enabled else 0)
                + _area_type_vehicle_penalty(vehicle_idx, to_node)
                + _approved_boundary_overflow_vehicle_penalty(vehicle_idx, to_node)
                + _return_home_penalty_cost(vehicle_idx, from_node, to_node)
            )
    ...
    uses_approved_boundary_overflow = any(
        _uses_own_region_boundary_policy(region_policy) and bool(codes)
        for region_policy, codes in zip(
            region_policy_by_job,
            approved_boundary_overflow_codes_by_job,
        )
    )
    # Region affinity uses a dedicated dimension so the 4,500 unit preference
    # is retained by every search strategy, including the initial insertion.
    uses_preferred_region_soft_penalty = any(preferred_region_by_vehicle) and any(region_names_by_job)
    uses_strict_area_type_preference = any(
        enforce and area_type in {"DMS", "DMS_CORE", "DMS_ONLY", "OVERLAP", "OVERLAB"}
        for enforce, area_type in zip(
            enforce_area_type_center_match_by_job,
            area_types_by_job,
        )
    )
    uses_vehicle_specific_cost = (
        priority_load_enabled
        or uses_approved_boundary_overflow
        or uses_strict_area_type_preference
    )
    if uses_vehicle_specific_cost:
        for vehicle_idx in range(vehicle_count):
    ...
    if uses_preferred_region_soft_penalty:
        preferred_region_callback_indices = [
            routing.RegisterTransitCallback(
                lambda from_index, to_index, vehicle_idx=vehicle_idx: _preferred_region_vehicle_penalty(
                    vehicle_idx,
        ...
        routing.AddDimensionWithVehicleTransits(
            preferred_region_callback_indices,
            0,
            max(1, int(VRP_PREFERRED_REGION_MISMATCH_PENALTY_COST) * int(job_count)),
            True,
            "PreferredRegionAffinity",
        )
        preferred_region_dimension = routing.GetDimensionOrDie("PreferredRegionAffinity")
        for vehicle_idx in range(vehicle_count):
            preferred_region_dimension.SetCumulVarSoftUpperBound(
                routing.End(vehicle_idx),
                0,
                1,
            )
    ...
    for job_idx, (_, row) in enumerate(job_df.iterrows()):
        ...
        fixed_vehicle_idx = vehicle_index_by_code.get(fixed_employee_code) if fixed_employee_code else None
        is_fixed_job = respect_fixed_jobs and _coerce_bool_value(row.get("fixed", False))
        is_fixed_outside_active_plan = (
            is_fixed_job
            and fixed_vehicle_idx is not None
            and _fixed_technician_outside_active_plan(row, fixed_employee_code)
        )
        is_respected_fixed_job = (
            is_fixed_job
            and fixed_vehicle_idx is not None
            and not is_fixed_outside_active_plan
        )
        is_unavailable_fixed_job = is_fixed_job and fixed_vehicle_idx is None
        is_reschedule_job = (
            enforce_reschedule_jobs
        ...
        if is_unavailable_fixed_job:
            ...
            # An available fixed technician outside the active plan may not be
            # assigned.  Release only that fixed constraint and retain the
            # job's normal hard candidate/capability/region policy below.
            ...
            if is_reschedule_job and not allowed_vehicle_indices and not has_explicit_hard_candidates:
                allowed_vehicle_indices = list(range(vehicle_count))
        if (
            0 <= job_idx < len(enforce_area_type_center_match_by_job)
            and enforce_area_type_center_match_by_job[job_idx]
        ):
            required_center_type = area_types_by_job[job_idx]
            dms_fallback_allowed = (
                0 <= job_idx < len(area_type_dms_fallback_allowed_by_job)
                and area_type_dms_fallback_allowed_by_job[job_idx]
            )
            if (
                required_center_type in {"DMS", "DMS_CORE", "DMS_ONLY"}
                and not dms_fallback_allowed
            ):
                allowed_vehicle_indices = [
                    int(vehicle_idx)
                    for vehicle_idx in allowed_vehicle_indices
                    if center_type_by_vehicle[int(vehicle_idx)] == base.DMS_CENTER_TYPE
                ]
            elif required_center_type in {"DMS2", "DMS2_EXCLUSIVE", "DMS2_ONLY"}:
                allowed_vehicle_indices = [
                    int(vehicle_idx)
                    for vehicle_idx in allowed_vehicle_indices
                    if center_type_by_vehicle[int(vehicle_idx)] == base.DMS2_CENTER_TYPE
                ]
        ...
        elif is_reschedule_like_priority_job and not is_respected_fixed_job:
            allowed_vehicle_indices = [
                int(vehicle_idx)
                for vehicle_idx in allowed_vehicle_indices
        ...
        if not allowed_vehicle_indices:
            ...
            routing.VehicleVar(node_index).SetValues([-1])
    ...
    for vehicle_idx, engineer_code in enumerate(vehicle_codes):
        ...
        while not routing.IsEnd(index):
            ...
            if node < job_count:
                ...
                row_dict["fixed_technician_outside_active_plan_relaxed"] = bool(
                    respect_fixed_jobs
                    and _coerce_bool_value(job_row.get("fixed", False))
                    and _fixed_technician_outside_active_plan(job_row, job_row.get("current_employee_code", ""))
                )
