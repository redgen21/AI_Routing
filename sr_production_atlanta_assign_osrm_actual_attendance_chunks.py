...
def main() -> None:
    ...
    for idx, chunk in enumerate(chunks, start=1):
        suffix = f"osrm_actual_attendance_chunk_{idx:02d}"
    ...
    assignment_df.to_csv(OUT_DIR / "atlanta_assignment_result_osrm_actual_attendance.csv", index=False, encoding="utf-8-sig")
    engineer_day_summary_df.to_csv(OUT_DIR / "atlanta_engineer_day_summary_osrm_actual_attendance.csv", index=False, encoding="utf-8-sig")
    schedule_df.to_csv(OUT_DIR / "atlanta_schedule_osrm_actual_attendance.csv", index=False, encoding="utf-8-sig")

    line_assignment_df = pd.read_csv(OUT_DIR / "atlanta_assignment_result_actual_attendance.csv", low_memory=False)
    line_summary_df = pd.read_csv(OUT_DIR / "atlanta_engineer_day_summary_actual_attendance.csv", low_memory=False)

    line_daily = _daily_metrics(line_assignment_df, line_summary_df)
    osrm_daily = _daily_metrics(assignment_df, engineer_day_summary_df)
    ...
    daily_compare_df.to_csv(OUT_DIR / "atlanta_daily_compare_line_actual_vs_osrm_actual.csv", index=False, encoding="utf-8-sig")

    print("merged_osrm_actual_attendance_outputs_ready")
