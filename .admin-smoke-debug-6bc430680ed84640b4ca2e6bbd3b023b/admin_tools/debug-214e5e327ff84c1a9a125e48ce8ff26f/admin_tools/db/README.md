...
- `migrations/`: ordered, versioned schema and least-privilege lifecycle
  migrations.
...
### Common migration runner

`migration_runner.py` is the operational CLI for **all** reviewed schema
changes. It reads only the immutable `migrations/manifest.json` allowlist;
operators cannot supply SQL or a migration path. Existing entries remain
development-only unless their reviewed manifest entry explicitly includes
`"target_environments": ["production"]`.

Production application requires a clean, separately approved Admin Tools
artifact, a successful preview, the exact typed confirmation shown by that
preview, `--confirm-production`, and non-secret approval and backup references.
It uses the existing transaction, advisory lock, checksum, and
`admin_schema_migration_history` protections. A DB migration never activates a
Region Plan or enables its runtime feature gate.

```bash
# Read-only registry/plan inspection; no database mutation occurs.
python -m admin_tools.db.migration_runner --json preview \
  --config /home/csda/AI_Routing/production/config_common_vrp.json \
  --migration-id V006__reviewed_change

# Only after preview, backup, approval, and release verification gates.
python -m admin_tools.db.migration_runner --json apply \
  --config /home/csda/AI_Routing/production/config_common_vrp.json \
  --migration-id V006__reviewed_change \
  --confirmation "APPLY V006__reviewed_change TO PRODUCTION vrp_db" \
  --confirm-production --approval-reference CHG-1234 --backup-reference backup-1234
```
