...
"""
RECEIPT_TABLE_SQL = """
create table if not exists admin_schema_migration_receipt (
...
def _migration_receipt_metadata(value: Mapping[str, Any] | None) -> dict[str, str]:
    if value is None:
        return {}
    if not isinstance(value, Mapping):
        raise ValueError("Migration receipt metadata must be an object.")
    allowed = {
        "execution_id", "approval_reference", "backup_reference", "artifact_manifest_sha256",
        "preview_token",
    }
    if set(value).difference(allowed):
        raise ValueError("Migration receipt metadata contains an unsupported field.")
    result: dict[str, str] = {}
    for key, raw in value.items():
        text = str(raw).strip()
        if not text or len(text) > 256 or any(ord(char) < 32 for char in text):
            raise ValueError("Migration receipt metadata is invalid.")
        result[str(key)] = text
    return result


def _insert_migration_receipt(
    cursor: Any,
    receipt_id: str,
    plan: MigrationPlan,
    checksum: str,
    ...
    cursor.execute(
        """
        insert into admin_schema_migration_receipt
            (receipt_id, migration_id, checksum_sha256, target_environment, target_dbname, status, metadata)
        values (%s, %s, %s, %s, %s, %s, %s::jsonb)
        """,
        (
            receipt_id, plan.migration_id, checksum, plan.target.environment, plan.target.dbname,
            status, json.dumps(dict(metadata), ensure_ascii=False, sort_keys=True),
        ),
    )
...
class DatabaseReleaseBackend:
    ...
    def apply(
        ...
        *,
        ...
        metadata = _migration_receipt_metadata(receipt_metadata)
        receipt_id = str(metadata.get("execution_id") or uuid.uuid4().hex)
        metadata["execution_id"] = receipt_id
        ...
        try:
            ...
            with connection.cursor() as cursor:
                ...
                cursor.execute(RECEIPT_TABLE_SQL)
                cursor.execute(
                ...
                _insert_migration_receipt(
                    cursor, receipt_id, plan, spec.checksum_sha256, "success", metadata
                )
            connection.commit()
            return MigrationResult(
                migration_id,
            ...
            try:
                self._record_failed_migration(
                    plan, spec, connection_factory, receipt_id, metadata, type(exc).__name__
                )
            except Exception:
                # Preserve the migration's original failure; a receipt write must
                # never mask the SQL/connection error that operators need to fix.
                pass
            raise
        finally:
            connection.close()
    ...
    def _record_failed_migration(
        ...
        """Persist failed history in a fresh transaction after SQL rollback."""
        connection = connection_factory(plan.target)
        try:
            if hasattr(connection, "autocommit"):
                connection.autocommit = False
            with connection.cursor() as cursor:
                cursor.execute("select set_config('statement_timeout', %s, true)", (str(self.statement_timeout_ms),))
                cursor.execute("select pg_advisory_xact_lock(%s)", (self.advisory_lock_key,))
                cursor.execute(HISTORY_TABLE_SQL)
                cursor.execute(RECEIPT_TABLE_SQL)
                rollback_metadata = json.dumps({
                ...
                cursor.execute(
                    ...
                    insert into admin_schema_migration_history
                        (migration_id, description, checksum_sha256, status, rollback_metadata, applied_at)
                    values (%s, %s, %s, 'failed', %s::jsonb, now())
                    on conflict (migration_id) do update set
                        description = excluded.description, checksum_sha256 = excluded.checksum_sha256,
                        status = excluded.status, rollback_metadata = excluded.rollback_metadata,
                        applied_at = excluded.applied_at
                    """,
                    (spec.migration_id, spec.description, spec.checksum_sha256, rollback_metadata),
                )
                _insert_migration_receipt(
                    cursor, receipt_id, plan, spec.checksum_sha256, "failed", metadata
                )
            connection.commit()
        finally:
            connection.close()
