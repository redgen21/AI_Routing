...
# Administrative releases never create or migrate the runtime schema. These
# are the exact table, column, and primary-key contracts needed by each write
# operation. ``verify_admin_schema`` checks them in a read-only transaction
# before a command opens its write transaction.
ADMIN_SCHEMA_CONTRACTS: dict[str, dict[str, tuple[tuple[str, ...], tuple[str, ...]]]] = {
    "reset": {
        "common_routing_result": (("request_id",), ("request_id",)),
        "common_routing_request": (("request_id", "subsidiary_name", "strategic_city_name", "promise_date"), ("request_id",)),
        "common_request_technician_input": (("record_id", "subsidiary_name", "strategic_city_name", "promise_date"), ("record_id",)),
    ...
    "la_seed": {
        ...
        "common_heavy_repair_rule_master": (("product_group_code", "product_code", "detailed_symptom_code"), ("product_group_code", "product_code", "detailed_symptom_code")),
    },
    "asia_import": {
        "common_technician_master": (("subsidiary_name", "strategic_city_name", "employee_code", "employee_name", "center_type", "home_address", "home_city", "home_state", "home_country", "home_postal_code", "home_latitude", "home_longitude", "active_flag", "priority_group", "max_home_to_job_min", "updated_at"), ("subsidiary_name", "strategic_city_name", "employee_code")),
        "common_technician_capability_master": (("subsidiary_name", "strategic_city_name", "employee_code", "product_group_code", "product_code", "repair_allowed", "heavy_repair_allowed", "priority_score", "effective_start_date", "effective_end_date", "updated_at"), ("subsidiary_name", "strategic_city_name", "employee_code", "product_group_code", "product_code")),
        "common_region_master": (("subsidiary_name", "strategic_city_name", "postal_code", "region_seq", "region_name", "region_center_latitude", "region_center_longitude", "updated_at"), ("subsidiary_name", "strategic_city_name", "postal_code")),
    },
}
...
def verify_admin_schema(config_path: Path, *, operation: str) -> None:
    """Fail closed when a canonical write target is absent or legacy-shaped.

    This executes only catalog queries in an explicit read-only transaction. A
    reviewed migration, not an administrative seed/import release, owns any
    repair needed to satisfy this contract.
    """
    try:
        contract = ADMIN_SCHEMA_CONTRACTS[operation]
    except KeyError as exc:
        raise ValueError(f"Unknown administrative schema operation: {operation}") from exc
    table_names = sorted(contract)
    connection = get_db_connection(config_path)
    try:
        with connection.cursor() as cursor:
            ...
            actual_primary_keys: dict[str, list[str]] = {table: [] for table in table_names}
            for table_name, column_name in cursor.fetchall():
                if table_name in actual_primary_keys:
                    actual_primary_keys[table_name].append(column_name)
        errors: list[str] = []
        for table_name, (required_columns, primary_key) in contract.items():
            missing = sorted(set(required_columns) - actual_columns[table_name])
            if missing:
                errors.append(f"{table_name}: missing columns {', '.join(missing)}")
            if tuple(actual_primary_keys[table_name]) != primary_key:
                current = ", ".join(actual_primary_keys[table_name]) or "<none>"
                errors.append(f"{table_name}: primary key is ({current}), expected ({', '.join(primary_key)})")
        if errors:
            raise RuntimeError(
                "ADMIN_SCHEMA_INCOMPATIBLE: apply the reviewed migration before writing: "
                + "; ".join(errors)
            )
    finally:
        # The catalog inspection must leave no transaction to commit and never
        # shares its connection with the subsequent write transaction.
        connection.rollback()
        connection.close()
...
def seed_default_masters(
    config_path: Path = COMMON_CONFIG_PATH, *, connection: Any | None = None,
) -> None:
