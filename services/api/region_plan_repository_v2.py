    ...
    def review(self, r: Mapping[str, Any], **_):
        ...
        try:
            with conn.cursor() as c:
                self._begin(c,r)
                c.execute("select p.revision,cc.activation_revision,p.plan_status,p.policy_version,p.source_sha256,p.manifest_sha256,p.bundle_sha256,p.membership_accepted_rows,p.membership_rejected_rows,p.unique_postal_count,p.technician_count,cc.source_strategic_city_name,p.verified_content_sha256 from common_region_plan p join common_city_context cc using(subsidiary_name,strategic_city_name) where p.subsidiary_name=%s and p.strategic_city_name=%s and p.plan_id=%s for update",(identity.subsidiary_name,identity.strategic_city_name,identity.plan_id)); current=c.fetchone()
    ...
    def preview(self,r: Mapping[str,Any],**_):
        ...
        try:
            with conn.cursor() as c:
                ...
                c.execute("select p.plan_status,p.policy_version,p.source_sha256,p.manifest_sha256,p.bundle_sha256,p.membership_accepted_rows,p.membership_rejected_rows,p.unique_postal_count,p.technician_count,cc.source_strategic_city_name,p.verified_content_sha256 from common_region_plan p join common_city_context cc using(subsidiary_name,strategic_city_name) where p.subsidiary_name=%s and p.strategic_city_name=%s and p.plan_id=%s for share",(identity.subsidiary_name,identity.strategic_city_name,identity.plan_id)); stored=c.fetchone()
            ...
            with conn.cursor() as c:
                ...
                if c.fetchone()[0] is not None:
                    ...
                    if normalized_plan:
                        ...
                        c.execute("update common_region_set rs set status='active',updated_at=now() from common_routing_plan rp where rp.subsidiary_name=%s and rp.strategic_city_name=%s and rp.routing_plan_id=%s and rs.subsidiary_name=rp.subsidiary_name and rs.source_strategic_city_name=rp.source_strategic_city_name and rs.region_set_id=rp.region_set_id", (identity.subsidiary_name, identity.strategic_city_name, identity.plan_id))
