...
DEFAULT_SUBSIDIARY_NAME = "LGEID"
DEFAULT_STRATEGIC_CITY_NAME = "INDONESIA"
DEFAULT_STRATEGIC_CITY_OPTIONS = [DEFAULT_STRATEGIC_CITY_NAME, "THAILAND", "MALAYSIA"]
...
def _build_region_layers(strategic_city_name: str, region_zip_df: pd.DataFrame, service_df: pd.DataFrame):
    ...
    if region_zip_df.empty or not set(required_region_cols).issubset(region_zip_df.columns):
        coverage_df = pd.DataFrame(columns=required_region_cols)
    else:
        coverage_df = region_zip_df[required_region_cols].drop_duplicates().copy()
...
def build_map(
    ...
    if not zip_layer.empty:
        folium.GeoJson(
            ...
            style_function=lambda feature: {
                "color": "transparent",
                "weight": 0,
                "fillColor": "#eceff3" if int(feature["properties"].get("service_count", 0) or 0) == 0 else region_colors.get(feature["properties"].get("AREA_NAME", ""), "#dddddd"),
                "fillOpacity": 0.05 if int(feature["properties"].get("service_count", 0) or 0) == 0 else 0.12,
            },
            tooltip=folium.GeoJsonTooltip(fields=["POSTAL_CODE", "AREA_NAME", "service_count"], aliases=["ZIP", "Area", "Service Count"]),
        ).add_to(fmap)
    if not region_layer.empty:
        folium.GeoJson(
            ...
            name="Area",
            style_function=lambda feature: {
                "fillColor": region_colors.get(feature["properties"].get("AREA_NAME", ""), "#0f766e"),
                "color": region_colors.get(feature["properties"].get("AREA_NAME", ""), "#0f766e"),
                "weight": 3,
                "fillOpacity": 0.06,
            },
            ...
            tooltip=folium.GeoJsonTooltip(fields=["AREA_NAME"], aliases=["Area"]),
        ).add_to(fmap)
...
def _load_local_jobs(subsidiary_name: str, strategic_city_name: str) -> pd.DataFrame:
    ...
    if "reschedule" not in filtered.columns:
        filtered["reschedule"] = False
    filtered["reschedule"] = _coerce_bool_series(filtered["reschedule"])
    if "job_slot_count" not in filtered.columns:
        filtered["job_slot_count"] = filtered["two_slot_job"].map(lambda value: 2 if _coerce_bool_value(value) else 1) if "two_slot_job" in filtered.columns else 1
    filtered["job_slot_count"] = _coerce_job_slot_count_series(filtered["job_slot_count"])
    return filtered
...
def _save_local_jobs(subsidiary_name: str, strategic_city_name: str, new_rows_df: pd.DataFrame) -> None:
    ...
    if "reschedule" not in working_new_rows_df.columns:
        working_new_rows_df["reschedule"] = False
    working_new_rows_df["reschedule"] = _coerce_bool_series(working_new_rows_df["reschedule"])
    if "job_slot_count" not in working_new_rows_df.columns:
        working_new_rows_df["job_slot_count"] = working_new_rows_df["two_slot_job"].map(lambda value: 2 if _coerce_bool_value(value) else 1) if "two_slot_job" in working_new_rows_df.columns else 1
    working_new_rows_df["job_slot_count"] = _coerce_job_slot_count_series(working_new_rows_df["job_slot_count"])
    if "promise_date" in working_new_rows_df.columns:
        working_new_rows_df["promise_date"] = working_new_rows_df["promise_date"].astype(str)
    if "record_id" in working_new_rows_df.columns:
        working_new_rows_df["record_id"] = working_new_rows_df["record_id"].astype(str)
    ...
    if all_df.empty:
        ...
        if "fixed" not in all_df.columns:
            all_df["fixed"] = False
        if "reschedule" not in all_df.columns:
            all_df["reschedule"] = False
        if "job_slot_count" not in all_df.columns:
            all_df["job_slot_count"] = all_df["two_slot_job"].map(lambda value: 2 if _coerce_bool_value(value) else 1) if "two_slot_job" in all_df.columns else 1
        if "promise_date" in all_df.columns:
            all_df["promise_date"] = all_df["promise_date"].astype(str)
        if "record_id" in all_df.columns:
            all_df["record_id"] = all_df["record_id"].astype(str)
        replace_mask = (
            (all_df["subsidiary_name"].astype(str) == str(subsidiary_name))
            & (all_df["strategic_city_name"].astype(str) == str(strategic_city_name))
            & (all_df["promise_date"].astype(str).isin(working_new_rows_df["promise_date"].astype(str)))
            & (all_df["gsfs_receipt_no"].astype(str).isin(working_new_rows_df["gsfs_receipt_no"].astype(str)))
        )
        if "record_id" in all_df.columns and "record_id" in working_new_rows_df.columns:
            replace_mask = replace_mask | all_df["record_id"].astype(str).isin(working_new_rows_df["record_id"].astype(str))
        base_df = all_df.loc[~replace_mask].copy()
    merged_df = pd.concat([base_df, working_new_rows_df], ignore_index=True)
    merged_df["fixed"] = _coerce_bool_series(merged_df["fixed"])
    merged_df["reschedule"] = _coerce_bool_series(merged_df["reschedule"])
    merged_df["job_slot_count"] = _coerce_job_slot_count_series(merged_df["job_slot_count"])
...
def _delete_local_job(subsidiary_name: str, strategic_city_name: str, record_id: str) -> None:
    ...
    filtered_df = all_df[
        ~(
            (all_df["subsidiary_name"].astype(str) == str(subsidiary_name))
            & (all_df["strategic_city_name"].astype(str) == str(strategic_city_name))
            & (all_df["record_id"].astype(str) == str(record_id))
        )
    ].copy()
    if "fixed" in filtered_df.columns:
        filtered_df["fixed"] = _coerce_bool_series(filtered_df["fixed"])
    if "job_slot_count" in filtered_df.columns:
        filtered_df["job_slot_count"] = _coerce_job_slot_count_series(filtered_df["job_slot_count"])
    _write_local_parquet(COMMON_JOB_STORE_PATH, filtered_df)
...
def _load_local_technicians(subsidiary_name: str, strategic_city_name: str, promise_date: str) -> pd.DataFrame:
    ...
    if str(strategic_city_name).strip().upper() in {str(value).strip().upper() for value in DEFAULT_STRATEGIC_CITY_OPTIONS}:
...
def _save_local_technicians(subsidiary_name: str, strategic_city_name: str, promise_date: str, rows_df: pd.DataFrame) -> None:
    ...
    if all_df.empty:
        ...
        if str(strategic_city_name).strip().upper() not in {str(value).strip().upper() for value in DEFAULT_STRATEGIC_CITY_OPTIONS}:
            base_mask = base_mask & (all_df["strategic_city_name"].astype(str) == str(strategic_city_name))
        base_df = all_df[~base_mask].copy()
...
def _load_profile_home_address_df(profile_path: str = str(PROFILE_SOURCE_PATH)) -> pd.DataFrame:
    return _load_excel_engineer_master_df(profile_path)


def _normalize_asia_center_type(value: object, default: str = "DSC") -> str:
...
def _normalize_asia_product_group(value: object) -> str:
    text = _clean_text(value)
    upper = text.upper()
    mapping = {
        "RAC": "ACN",
...
@st.cache_data(show_spinner=False)
def _load_excel_engineer_master_df(profile_path: str = str(PROFILE_SOURCE_PATH)) -> pd.DataFrame:
    path = Path(profile_path)
    if not path.exists():
        return pd.DataFrame(
            columns=[
    ...
    slot_df = pd.read_excel(path, sheet_name="2. Slot").rename(
        columns={
            ...
            "Slot": "slot_count",
            "STRATEGIC_CITY_NAME": "profile_city_name",
        }
    )
    ...
    slot_cols = ["employee_code", "employee_name", "center_type", "slot_count", "profile_city_name"]
    address_cols = [
        "employee_code",
        "employee_name",
        "home_address",
    ...
    working["employee_name"] = working["employee_name"].fillna(working.get("employee_name_address"))
    for col in ["employee_code", "employee_name", "center_type", "profile_city_name", "home_address", "home_city", "home_state", "home_postal_code"]:
        working[col] = working[col].map(_clean_text)
    working["center_type"] = working["center_type"].map(_normalize_asia_center_type)
    working["strategic_city_name"] = working["profile_city_name"].where(working["profile_city_name"].ne(""), working["home_city"])
    working["home_country"] = working["home_state"].replace("", "Indonesia")
    working["home_postal_code"] = working["home_postal_code"].str.replace(r"\.0+$", "", regex=True)
    working["home_latitude"] = pd.to_numeric(working["home_latitude"], errors="coerce")
    working["home_longitude"] = pd.to_numeric(working["home_longitude"], errors="coerce")
    working["slot_count"] = pd.to_numeric(working["slot_count"], errors="coerce").fillna(8).astype(int)
    working["active_flag"] = True
    working["priority_group"] = "A"
    working["max_minutes"] = 540
    return working.drop(columns=["employee_name_address"], errors="ignore")
...
def _normalize_technician_rows(
    ...
    *,
    ...
    for _, row in raw_df.iterrows():
        ...
        slot_count = pd.to_numeric(row.get("slot_count", master_row.get("slot_count", 8)), errors="coerce")
        priority_group = _coerce_priority_group_value(row.get("priority_group", master_row.get("priority_group", "B")))
        max_minutes = pd.to_numeric(row.get("max_minutes", master_row.get("max_minutes", 540)), errors="coerce")
        ...
        row_strategic_city_name = (
            _clean_text(master_row.get("strategic_city_name"))
            or _clean_text(row.get("strategic_city_name"))
            or strategic_city_name
        )
        home_latitude = pd.to_numeric(row.get("home_latitude", master_row.get("home_latitude")), errors="coerce")
        home_longitude = pd.to_numeric(row.get("home_longitude", master_row.get("home_longitude")), errors="coerce")
        normalized_rows.append(
            {
                "record_id": _clean_text(row.get("record_id")) or uuid.uuid4().hex,
                "subsidiary_name": subsidiary_name,
                "strategic_city_name": row_strategic_city_name,
                ...
                "center_type": _normalize_asia_center_type(_clean_text(row.get("center_type")) or _clean_text(master_row.get("center_type"))),
                "available": available,
                ...
                "home_address": _clean_text(master_row.get("home_address")) or _clean_text(row.get("home_address")),
                "home_city": _clean_text(master_row.get("home_city")) or _clean_text(row.get("home_city")),
                "home_state": _clean_text(master_row.get("home_state")) or _clean_text(row.get("home_state")),
                "home_country": _clean_text(master_row.get("home_country")) or _clean_text(row.get("home_country")) or "Indonesia",
                "home_postal_code": _clean_text(master_row.get("home_postal_code")) or _clean_text(row.get("home_postal_code")),
                "home_latitude": float(home_latitude) if pd.notna(home_latitude) else pd.NA,
                "home_longitude": float(home_longitude) if pd.notna(home_longitude) else pd.NA,
...
def _build_capability_rows_for_payload(
    ...
    if filtered.empty:
        return []
    requested_groups = {group for group, _product in requested_products}
    filtered["SERVICE_PRODUCT_GROUP_CODE"] = filtered["SERVICE_PRODUCT_GROUP_CODE"].map(_normalize_asia_product_group)
    # Asia profile currently has mixed product descriptions in SERVICE_PRODUCT_CODE.
    # For the first Asia test run, capability matching is intentionally group-only.
    filtered["SERVICE_PRODUCT_CODE"] = ""
    filtered = filtered[
        filtered["SERVICE_PRODUCT_GROUP_CODE"].astype(str).str.strip().isin(requested_groups)
    ].copy()
    records: list[dict] = []
    ...
    covered_groups = {
        ...
        for row in records
        if str(row.get("product_group_code", "")).strip()
    }
    missing_requested_groups = sorted(group for group in requested_groups if group and group not in covered_groups)
            ...
            [
                {
                    ...
                    "source": "temporary_missing_group_fallback",
                }
                for group in missing_requested_groups
            ]
        )
    return records
...
def _prepare_jobs_df(
    ...
    *,
    ...
    working["STATE_NAME"] = str(strategic_city_name).strip().title()
    working["COUNTRY_NAME"] = str(strategic_city_name).strip().title()
...
def _build_common_actual_frames(
    ...
    actual_df["assigned_center_type"] = (
        actual_df["assigned_center_type"].fillna("").astype(str).str.strip().replace("", "DMS").str.upper()
    )
    if not region_zip_df.empty:
        region_lookup = region_zip_df[["POSTAL_CODE", "region_seq", "new_region_name"]].drop_duplicates().copy()
        region_lookup["POSTAL_CODE"] = region_lookup["POSTAL_CODE"].astype(str).str.zfill(5)
...
def _build_result_view_state(subsidiary_name: str, strategic_city_name: str) -> dict | None:
    ...
    engineer_master_df = _load_excel_engineer_master_df()
    ...
    available_regions = ["ALL"] + sorted(schedule_regions | unassigned_regions)
...
def _render_result_summary(subsidiary_name: str, strategic_city_name: str) -> None:
    ...
    if state.get("compare_mode") == "Smart Routing":
        ...
        st.metric(
            "Service Count",
            f"{total_service_count} (Assigned {state['service_count']} / Not Assigned {state.get('unassigned_count', 0)})",
        )
    else:
    ...
    st.metric("Average Distance (mile)", f"{state['avg_distance'] * KM_TO_MILES:.2f}")
...
def _build_default_technician_rows_from_jobs(jobs_df: pd.DataFrame, engineer_master_df: pd.DataFrame, subsidiary_name: str, strategic_city_name: str) -> list[dict]:
    ...
    for _, row in unique_jobs.iterrows():
        ...
        center_type = _normalize_asia_center_type(master_row.get("center_type"))
        active_flag = _coerce_bool_value(master_row.get("active_flag", True)) if is_master_technician else False
        row_strategic_city_name = _clean_text(master_row.get("strategic_city_name")) or strategic_city_name
        rows.append(
            {
                ...
                "shift_start": "09:00",
                "shift_end": "18:00",
                "slot_count": int(pd.to_numeric(pd.Series([master_row.get("slot_count", 8)]), errors="coerce").fillna(8).iloc[0]),
                "max_minutes": int(pd.to_numeric(pd.Series([master_row.get("max_minutes", 540)]), errors="coerce").fillna(540).iloc[0]),
...
def _build_default_technician_rows_from_master(engineer_master_df: pd.DataFrame, subsidiary_name: str, strategic_city_name: str) -> list[dict]:
    ...
    for _, row in engineer_master_df.drop_duplicates(subset=["employee_code"]).iterrows():
        ...
        slot_count = pd.to_numeric(pd.Series([row.get("slot_count", master_row.get("slot_count", 8))]), errors="coerce").fillna(8).iloc[0]
        max_minutes = pd.to_numeric(pd.Series([row.get("max_minutes", master_row.get("max_minutes", 540))]), errors="coerce").fillna(540).iloc[0]
        row_strategic_city_name = _clean_text(row.get("strategic_city_name")) or _clean_text(master_row.get("strategic_city_name")) or strategic_city_name
        rows.append(
            {
                "record_id": uuid.uuid4().hex,
                "subsidiary_name": subsidiary_name,
                ...
                "center_type": _normalize_asia_center_type(row.get("center_type")),
                "shift_start": "09:00",
                "shift_end": "18:00",
                "slot_count": int(slot_count) if float(slot_count) >= 0 else 8,
                "max_minutes": int(max_minutes) if float(max_minutes) > 0 else 540,
...
def _technician_master_dialog(subsidiary_name: str, strategic_city_name: str) -> None:
    engineer_master_df = _load_excel_engineer_master_df()
    st.caption(str(PROFILE_SOURCE_PATH))
    if engineer_master_df.empty:
        st.info("No technician master rows in the Asia profile Excel file.")
    else:
        st.dataframe(engineer_master_df, width="stretch", hide_index=True)
    st.info("Asia technician master is loaded from Excel only. DB/API edits are disabled in this client.")
    return
...
def _render_jobs_tab(subsidiary_name: str, strategic_city_name: str) -> None:
    ...
    if jobs_df.empty:
        ...
        edited_jobs_df = st.data_editor(
            editor_source_df[editor_display_cols],
            width="stretch",
            hide_index=True,
            num_rows="fixed",
            disabled=[col for col in editor_display_cols if col not in {"select", "fixed", "job_slot_count"}],
            column_config={
...
def _render_technicians_tab(subsidiary_name: str, strategic_city_name: str) -> None:
    ...
    if manage_col.button("Excel Technician Master", type="secondary", width="stretch"):
        st.session_state[f"asia_technician_master_preview::{subsidiary_name}::{strategic_city_name}"] = True
    if st.session_state.get(f"asia_technician_master_preview::{subsidiary_name}::{strategic_city_name}"):
        with st.expander("Excel Technician Master Preview", expanded=True):
            st.caption(str(PROFILE_SOURCE_PATH))
            st.dataframe(engineer_master_df, width="stretch", hide_index=True)
...
def main() -> None:
    st.title("Smart Routing Client - Asia")
    subsidiaries = [DEFAULT_SUBSIDIARY_NAME]
    cities = DEFAULT_STRATEGIC_CITY_OPTIONS
