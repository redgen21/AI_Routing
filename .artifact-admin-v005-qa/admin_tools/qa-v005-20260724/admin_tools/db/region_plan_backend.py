...
def _generic_identity(request: Mapping[str, Any]) -> GenericPlanIdentity:
    ...
    if target == STRATEGIC_CITY_NAME or policy != GENERIC_POLICY_VERSION:
    ...
    if counts != GENERIC_EXPECTED_COUNTS:
...
class GenericRegionPlanLifecycleRepository:
    ...
    def _load_candidate(cursor: Any, identity: GenericPlanIdentity, *, allowed_statuses: tuple[str, ...]) -> tuple[int, int, str | None]:
        ...
        if (
            ...
            or (int(row[7]), int(row[8]), int(row[9]), int(row[10])) != (413, 0, 413, 54)
        ...
        if counts[:4] != GENERIC_EXPECTED_COUNTS:
            raise RegionPlanContractError("PLAN_ROW_COUNTS_INVALID")
        if counts[4:] != (0, 0):
...
def _build_cli_parser() -> argparse.ArgumentParser:
    ...
    migration_preview.add_argument("--migration-id", choices=(MIGRATION_ID, "V002__region_plan_unbounded_region_seq", "V003__region_plan_technician_source_id", "V004__region_plan_area_type_region_soft", "V005__region_plan_workflow_grants"), default=MIGRATION_ID)
    ...
    install_schema.add_argument("--migration-id", choices=(MIGRATION_ID, "V002__region_plan_unbounded_region_seq", "V003__region_plan_technician_source_id", "V004__region_plan_area_type_region_soft", "V005__region_plan_workflow_grants"), default=MIGRATION_ID)
