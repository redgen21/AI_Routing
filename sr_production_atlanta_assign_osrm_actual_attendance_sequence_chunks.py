...
def main() -> None:
    ...
    for idx, chunk in enumerate(chunks, start=1):
        suffix = f"osrm_actual_attendance_sequence_chunk_{idx:02d}"
    ...
    assignment_df.to_csv(OUT_DIR / "atlanta_assignment_result_osrm_actual_attendance_sequence.csv", index=False, encoding="utf-8-sig")
    engineer_day_summary_df.to_csv(OUT_DIR / "atlanta_engineer_day_summary_osrm_actual_attendance_sequence.csv", index=False, encoding="utf-8-sig")
    schedule_df.to_csv(OUT_DIR / "atlanta_schedule_osrm_actual_attendance_sequence.csv", index=False, encoding="utf-8-sig")

    print("merged_osrm_actual_attendance_sequence_outputs_ready")
