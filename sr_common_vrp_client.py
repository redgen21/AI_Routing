...
def _area_type_bucket(area_type: object) -> str:
    text = _clean_text(area_type).upper()
    if text in {"DMS", "DMS_CORE", "DMS_ONLY"}:
        return "DMS"
    if text in {"OVERLAP", "OVERLAB"}:
        return "OVERLAP"
...
def _area_type_layer_label(area_type: object) -> str:
    bucket = _area_type_bucket(area_type)
    return bucket or "Other"


def _ordered_area_type_values(layer: pd.DataFrame) -> list[str]:
    if layer.empty or "area_type" not in layer.columns:
        return []
    values = {_clean_text(value).upper() for value in layer["area_type"].dropna().tolist()}
    values = {value for value in values if value}
    preferred = ["DMS_CORE", "DMS", "OVERLAP", "DMS2_EXCLUSIVE", "DMS2"]
    ordered = [value for value in preferred if value in values]
    return ordered + sorted(values - set(ordered))
...
def _build_region_layers(strategic_city_name: str, region_zip_df: pd.DataFrame, service_df: pd.DataFrame):
    ...
    # Use all ZIPs in the fixed-region master, not only ZIPs present in the
    # current service/profile-derived map cache.
...
def _resequence_force_assigned_routes(result_payload: dict, affected_employee_codes: set[str]) -> dict:
    ...
    the Actual-style OSRM ordered route for ``home -> all stops -> home`` and
...
def _build_actual_technician_summary_df(
    actual_schedule_df: pd.DataFrame,
    strategic_city_name: str,
) -> pd.DataFrame:
    columns = [
        "Technician", "job_count", "slot_count", "route_distance_mile",
        "return_home_distance_mile", "service_time_min", "total_working_min",
    ...
    for _, group in actual_schedule_df.groupby("assigned_sm_code", dropna=True):
        ...
        rows.append({
            "Technician": name,
            "job_count": int(units["GSFS_RECEIPT_NO"].dropna().astype(str).nunique()),
            "slot_count": int(slots.sum()),
            "route_distance_mile": round(route_distance_km * KM_TO_MILES, 2),
            "return_home_distance_mile": round(return_distance_km * KM_TO_MILES, 2),
            "service_time_min": round(service_time_min, 2),
            "total_working_min": round(route_duration_min + service_time_min, 2),
            "total_day_duration_with_return_min": round(route_duration_min + service_time_min + return_duration_min, 2),
        })
    return pd.DataFrame(rows, columns=columns).sort_values("Technician").reset_index(drop=True)


def _build_combined_technician_summary_df(
    actual_df: pd.DataFrame,
    smart_df: pd.DataFrame,
    service_date: str,
) -> pd.DataFrame:
    metric_columns = [
        "job_count", "slot_count", "route_distance_mile", "return_home_distance_mile",
        "service_time_min", "total_working_min", "total_day_duration_with_return_min",
    ]
    actual = actual_df.rename(columns={col: f"Actual_{col}" for col in metric_columns}) if not actual_df.empty else pd.DataFrame(columns=["Technician"])
    smart = smart_df.rename(columns={col: f"Smart_Routing_{col}" for col in metric_columns}) if not smart_df.empty else pd.DataFrame(columns=["Technician"])
...
def _load_period_statistics(
    ...
    for promise_date in selected_dates:
        try:
            ...
            actual_jobs_df = payload_jobs_df.copy()
            actual_assignment_df, actual_schedule_df = _build_common_actual_frames(
                actual_jobs_df, engineer_master_df, region_zip_df
            )
            ...
            smart_summary_df = pd.DataFrame(smart_summary_rows)
            technician_summary = _build_combined_technician_summary_df(
                actual_summary_df, smart_summary_df,
    ...
    return {
        ...
        "technician_summary": pd.concat(technician_summary_frames, ignore_index=True) if technician_summary_frames else pd.DataFrame(),
