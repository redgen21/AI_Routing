...
def main() -> None:
    ...
    for idx, chunk in enumerate(chunks, start=1):
        suffix = f"actual_attendance_chunk_{idx:02d}"
    ...
    assignment_df.to_csv(OUT_DIR / "atlanta_assignment_result_actual_attendance.csv", index=False, encoding="utf-8-sig")
    engineer_day_summary_df.to_csv(OUT_DIR / "atlanta_engineer_day_summary_actual_attendance.csv", index=False, encoding="utf-8-sig")
    schedule_df.to_csv(OUT_DIR / "atlanta_schedule_actual_attendance.csv", index=False, encoding="utf-8-sig")

    print("merged_actual_attendance_outputs_ready")
