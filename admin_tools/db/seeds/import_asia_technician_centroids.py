...
def load_service_file(config_path: Path) -> Path:
    ...
    if not service_file:
        raise ValueError("Missing area_map_asia.service_file in config/config.json")
    ...
    *,
    ...
    for _, row in df.iterrows():
        if pd.isna(row.get("home_latitude")) or pd.isna(row.get("home_longitude")):
            continue
        saved += upsert_technician_master(
            row.to_dict(), config_path=common_config_path, connection=connection
        )
    return saved


def import_capability_rows(
    df: pd.DataFrame,
    common_config_path: Path,
    *,
    connection: Any | None = None,
...
def apply_master_imports(
    technician_df: pd.DataFrame,
    capability_df: pd.DataFrame,
    region_df: pd.DataFrame,
    common_config_path: Path,
) -> tuple[int, int, int]:
    """Apply all Asia masters atomically after a read-only schema preflight."""
    verify_admin_schema(common_config_path, operation="asia_import")
    connection = get_db_connection(common_config_path)
    try:
        saved_technicians = import_technician_rows(
            technician_df, common_config_path, connection=connection
        )
        saved_capabilities = import_capability_rows(
            capability_df, common_config_path, connection=connection
        )
        saved_regions = import_region_rows(
            region_df, common_config_path, connection=connection
        )
        connection.commit()
        return saved_technicians, saved_capabilities, saved_regions
    except Exception:
        connection.rollback()
        raise
    finally:
...
def main() -> None:
    ...
    parser.add_argument(
        "--runtime-root",
        default="",
        help="Application environment root used for relative config, input, and output paths.",
    )
    ...
    if args.apply:
        require_db_write_allowed(common_config_path, confirm_production=args.confirm_production)
        saved_technicians, saved_capabilities, saved_regions = apply_master_imports(
            df, capability_df, region_df, common_config_path
        )
