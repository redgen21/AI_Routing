...
- `common_vrp.py`: release-local config, schema bootstrap, master upsert, and
  default-seed repository used only by these offline commands.
...
The release-local repository contains the schema bootstrap required by its
offline reset/seed/import commands. It is intentionally independent of the
application runtime package. Runtime code and admin code must still preserve
the same Common VRP table contract; reviewed migrations remain the mechanism
for ordered production schema changes.
